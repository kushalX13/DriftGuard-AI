...
# Rule definitions: (rule_id, resource_type, service, base_severity, port_options, public_cidr_prob)
RULES = [
    ("DG-SG-OPEN-SSH", "aws_security_group", "sg", 0, [22], 0.85),   # critical, port 22, mostly public
    ("DG-SG-OPEN-HTTP", "aws_security_group", "sg", 2, [80, 443], 0.6),  # medium, 80/443
    ("DG-S3-NO-ENCRYPTION", "aws_s3_bucket", "s3", 1, [-1], 0.3),   # high, no port
    ("DG-RDS-NO-ENCRYPTION", "aws_db_instance", "rds", 1, [-1], 0.2),  # high
    ("DG-IAM-WILDCARD", "aws_iam_policy", "iam", 0, [-1], 0.1),      # critical
]
...
def generate_synthetic(n_rows: int = 2500, seed: int = RANDOM_SEED) -> pd.DataFrame:
    """Generate n_rows synthetic findings. Reproducible if seed is fixed."""
    rng = random.Random(seed)
    rows = []
    weights = [0.25, 0.25, 0.20, 0.15, 0.15]  # balance across rules

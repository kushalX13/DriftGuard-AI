...
# Load .env so OPENAI_API_KEY is available for --use-llm (no-op if dotenv missing)
try:
...
def explain(
    ...
    use_llm: bool = typer.Option(
        False,
        "--use-llm",
        help="Use OpenAI to rewrite explanations in a friendly, conversational way (requires OPENAI_API_KEY and pip install driftguard-ai[llm]).",
    ),
) -> None:
    """Attach remediation docs to findings (deterministic retrieval). Writes reports/explanations.json."""
    try:
        explain_run(findings_json, output, use_llm=use_llm)
...
def run(
    ...
    use_llm: bool = typer.Option(
        False,
        "--use-llm",
        help="Use OpenAI to humanize finding explanations (requires OPENAI_API_KEY and pip install driftguard-ai[llm]).",
    ),
) -> None:
    ...
    try:
        ...
        explain_run(findings_path, explanations_path, use_llm=use_llm)
        step = 3
        ml_ran = False
